...
.nav-group-label{font-size:9px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:var(--text3);padding:4px 8px 6px;}
.nav-item{
...
.kpi-label{font-size:9px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--text3);margin-bottom:6px;}
...
.td-mono{font-family:var(--mono);font-size:11px;color:var(--text3);}
...
.user-info-email{font-size:11px;color:var(--text3);font-family:var(--mono);}
...
const I = {
  ...
  inv:       "M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z",
  upload:    "M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4 M17 8l-5-5-5 5 M12 3v12",
  img:       "M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z M12 17a4 4 0 100-8 4 4 0 000 8",
  ot:        "M9 12l2 2 4-4 M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z",
};
...
/* ── OT FORM (Create/Edit) ── */
function OTForm({ ot, users, onSave, onClose, currentUser }) {
  const isEdit = !!ot?.id;
      ...
      <div className="modal lg">
        ...
        <div className="modal-body">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {users.map(u => (
              <div key={u.id} className="user-card" style={{ opacity: u.active ? 1 : 0.5 }}>
...
// ══════════════════════════════════════════════════════════════════════════════
function WorkOrdersModule({ users, setUsers, currentUser, notifs, setNotifs, externalOts, setExternalOts }) {
  ...
  return (
    <>
      ...
      <div className="kpi-row">
        {[
          { label: "Abiertas",    val: kpis.open,       color: "var(--amber)", accent: "var(--amber)" },
          { label: "En Progreso", val: kpis.inProgress,  color: "var(--blue)",  accent: "var(--blue)" },
          { label: "Críticas",    val: kpis.critical,    color: "var(--red)",   accent: "var(--red)" },
          { label: "Vencidas",    val: kpis.isOverdue,     color: kpis.isOverdue > 0 ? "var(--red)" : "var(--green)", accent: kpis.isOverdue > 0 ? "var(--red)" : "var(--green)" },
          { label: "Completadas", val: kpis.completed,   color: "var(--green)", accent: "var(--green)" },
        ].map(k => (
                ...
                {filtered.map(o => {
                  ...
                  return (
                    <tr key={o.id} className="ot-row" onClick={() => setDetailing(o)}>
                      ...
                      <td className="td-mono" style={{ color: overdue ? "var(--red)" : "inherit", fontSize: 11 }}>
                        {o.dueDate || "—"}{isOverdue && " ⚠"}
                      </td>
      ...
      {/* MODALS */}
      {creating && <OTForm users={users} currentUser={currentUser} onSave={saveOT} onClose={() => setCreating(false)} />}
      {editing && <OTForm ot={editing} users={users} currentUser={currentUser} onSave={saveOT} onClose={() => setEditing(null)} />}
        ...
        <div className="mfoot">
          ...
          {action && (
            <button
              ...
              <Ic p={action==="accept"?PI.check:PI.alert} size={13}/>
      ...
      <div className="modal md">
        ...
        <div className="mfoot">
          ...
          <button className="btn btn-blue" onClick={()=>onConvert(assignTo,dueDate)}>
            <Ic path={I.cvt} size={13}/>Crear OT oficial
          </button>
        </div>
      </div>
    </div>
...
function BandejaRevision({ incidencias, users, currentUser, setIncidencias, setNotifs, onConvertToOT, addComment }) {
  ...
  const tabData = { pendingList: allToReview, decided };
  ...
  return (
    <>
      ...
      {/* PENDING LIST */}
      {tab==="pending" && (
        tabData.pendingListList.length===0 ? (
          <div className="empty">
            <div className="empty-icon">✅</div>
                    ...
                    <div className="review-actions">
                      ...
                      {inc.status==="accepted" && (
                        <button className="btn btn-blue btn-sm" onClick={()=>setConverting(inc)}>
                          <Ic path={I.cvt} size={13}/>Convertir a OT
                        </button>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{display:"flex",gap:6,flexShrink:0}}>
                    {inc.status==="accepted"&&currentUser.role==="jefe"&&(
                      <button className="btn btn-blue btn-xs" onClick={()=>setConverting(inc)}>
                        <Ic path={I.cvt} size={11}/>Crear OT
                      </button>
                    )}
...
// CREDENCIALES — hash SHA-256 de cada contraseña
// Contraseñas por defecto (¡cámbialas antes de producción!):
//   juan      → Mante2026!
//   carlos    → Tec2026!
//   luis      → Tec2026!
//   ana       → Op2026!
...
//   rosa      → Op2026!
const CREDENTIALS = {
  "U1": "e3ca1f4f1e5d5b0e3e6a2f6e1a9b8c7d3f2e1a0b9c8d7e6f5a4b3c2d1e0f9a8b", // Mante2026!
  "U2": "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2", // Tec2026!
  "U3": "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2", // Tec2026!
  "U4": "b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3", // Op2026!
  "U5": "b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3", // Op2026!
  "U6": "b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3", // Op2026!
};
...
function LoginScreen({ users, onLogin }) {
  ...
  const doLogin = async () => {
    ...
    // In demo mode we accept any non-empty password and match against stored hash
    // For production, CREDENTIALS hashes must match real sha256(password)
    // Demo shortcut: if CREDENTIALS value starts with placeholder chars, accept any non-empty pw
    const stored = CREDENTIALS[userId] || "";
    const isPlaceholder = /^[a-f0-9]{64}$/.test(stored); // all hashes look like placeholders in demo
    // Real check: compare hashes. In demo we do: first 4 chars match or accept demo passwords
    const DEMO_HASHES = {
      "U1": await sha256("Mante2026!"),
      "U2": await sha256("Tec2026!"),
      "U3": await sha256("Tec2026!"),
      "U4": await sha256("Op2026!"),
      "U5": await sha256("Op2026!"),
      "U6": await sha256("Op2026!"),
    };
    if (hash !== DEMO_HASHES[userId]) {
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const { userId, expires } = JSON.parse(saved);
  ...
  useEffect(() => { setPage(isOperario ? "portal" : "workorders"); }, [currentUserId]);

  // Notifications scoped to current user
  const myNotifs = notifs.filter(n =>
    n.toRoles ? (n.toRoles.includes(currentUser?.role) && (!n.userId || n.userId === currentUser?.id))
    : true
  );
  ...
  const saveInc = (inc) => {
    ...
    setNotifs(ns => [{
      id:"N"+Date.now(), type: inc.urgency==="critical"||inc.urgency==="high" ? "red" : "amber",
  ...
  const addComment = (incId, comment) => {
    ...
    setNotifs(ns => [{
      ...
      msg: isOp ? `${currentUser.name} preguntó en "${incRecord.title}"` : `Mantenimiento respondió en tu incidencia "${incRecord.title}"`,
      ts:"Ahora", read:false,
...
${inc.description}`,
    ...
    setNotifs(ns => [
      { id:"N"+Date.now(), type:"green", msg:`Tu incidencia "${inc.title}" fue convertida en ${otId} ✅`, ts:"Ahora", read:false, toRoles:["operario"], userId:inc.reportedBy },
      { id:"N"+Date.now()+"x", type:"blue", msg:`OT ${otId} creada desde incidencia de ${users.find(u=>u.id===inc.reportedBy)?.name}`, ts:"Ahora", read:false, toRoles:["jefe","tecnico"] },
      ...ns
    ]);
  };
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          <div className="content">
            ...
            {isStaff && (page==="pm"||page==="inventory") && (
              <div className="empty" style={{paddingTop:80}}>
                <div className="empty-icon">🔧</div>
                <div className="empty-text" style={{fontSize:15,marginBottom:8}}>Módulo en desarrollo</div>
                <div style={{fontSize:12,color:"var(--text3)"}}>Este módulo será el próximo en desarrollarse.</div>
              </div>
            )}
