...
/* ── PLANNING MODULE ── */
.plan-tabs{display:flex;gap:4px;margin-bottom:20px;background:var(--bg3);padding:4px;border-radius:8px;width:fit-content;}
.plan-tab{padding:7px 16px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;color:var(--text2);transition:all .15s;}
.plan-tab.on{background:var(--bg5);color:var(--text);box-shadow:0 1px 4px rgba(0,0,0,.3);}
.plan-tab:hover:not(.on){color:var(--text);}

/* Agenda */
.agenda-nav{display:flex;align-items:center;gap:10px;margin-bottom:16px;}
.agenda-nav-btn{background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:5px 10px;color:var(--text2);cursor:pointer;font-size:13px;transition:all .15s;}
.agenda-nav-btn:hover{border-color:var(--amber);color:var(--amber);}
.agenda-date-label{font-family:var(--fontc);font-size:14px;font-weight:700;color:var(--text);min-width:220px;text-align:center;}
.agenda-today-btn{background:var(--amber-d);border:1px solid var(--amber-g);border-radius:6px;padding:5px 12px;color:var(--amber);cursor:pointer;font-size:11px;font-weight:700;}

.agenda-cols{display:flex;gap:12px;}
.agenda-col{flex:1;min-width:0;}
.agenda-col-head{font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text2);padding:6px 10px;margin-bottom:8px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px;}
.agenda-col-head.today{color:var(--amber);}
.agenda-col-head-date{font-family:var(--mono);font-size:10px;color:var(--text3);}

.agenda-slot{background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:12px;margin-bottom:8px;cursor:pointer;transition:all .15s;position:relative;overflow:hidden;}
.agenda-slot::before{content:"";position:absolute;left:0;top:0;bottom:0;width:3px;}
.agenda-slot.critical::before{background:var(--red);}
.agenda-slot.high::before{background:var(--orange);}
.agenda-slot.medium::before{background:var(--yellow);}
.agenda-slot.low::before{background:var(--green);}
.agenda-slot:hover{border-color:var(--border2);transform:translateX(2px);}
.agenda-slot-id{font-family:var(--mono);font-size:9px;color:var(--text3);margin-bottom:3px;}
.agenda-slot-title{font-size:12px;font-weight:600;color:var(--text);margin-bottom:6px;line-height:1.3;}
.agenda-slot-meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap;}
.agenda-slot-hours{font-family:var(--mono);font-size:10px;background:var(--bg4);padding:1px 7px;border-radius:4px;color:var(--text2);}
.agenda-slot-asset{font-size:10px;color:var(--text3);}
.agenda-empty{text-align:center;padding:32px 16px;color:var(--text3);font-size:12px;border:1px dashed var(--border);border-radius:8px;}
.agenda-empty-icon{font-size:28px;margin-bottom:8px;}

/* Calendar */
.cal-nav{display:flex;align-items:center;gap:12px;margin-bottom:16px;}
.cal-month-label{font-family:var(--fontc);font-size:16px;font-weight:800;color:var(--text);min-width:180px;text-align:center;}
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:2px;}
.cal-dow{text-align:center;font-size:9px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--text3);padding:8px 4px;}
.cal-day{background:var(--bg2);border:1px solid var(--border);border-radius:6px;padding:6px;min-height:72px;cursor:pointer;transition:all .15s;position:relative;}
.cal-day:hover{border-color:var(--border2);}
.cal-day.today{border-color:var(--amber);background:var(--amber-d);}
.cal-day.other-month{opacity:.35;}
.cal-day.selected{border-color:var(--blue);background:var(--blue-d);}
.cal-day-num{font-size:11px;font-weight:700;color:var(--text2);margin-bottom:4px;font-family:var(--mono);}
...
.cal-day-count{font-size:9px;color:var(--text3);margin-top:3px;}
.cal-load-bar{position:absolute;bottom:0;left:0;right:0;height:2px;border-radius:0 0 6px 6px;transition:width .3s;}

/* Capacity */
.cap-table{width:100%;border-collapse:collapse;}
.cap-th{text-align:left;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text3);padding:8px 12px;border-bottom:2px solid var(--border);white-space:nowrap;}
.cap-th.center{text-align:center;}
.cap-td{padding:8px 12px;border-bottom:1px solid var(--border);font-size:12px;vertical-align:middle;}
.cap-td.center{text-align:center;}
.cap-tech{display:flex;align-items:center;gap:8px;}
.cap-bar-wrap{background:var(--bg4);border-radius:4px;height:6px;width:100%;overflow:hidden;}
.cap-bar{height:100%;border-radius:4px;transition:width .4s;}
.cap-bar.ok{background:var(--green);}
.cap-bar.warn{background:var(--yellow);}
.cap-bar.over{background:var(--red);}
.cap-hours{font-family:var(--mono);font-size:11px;white-space:nowrap;}
.cap-over{color:var(--red);font-weight:700;}
.cap-week-nav{display:flex;align-items:center;gap:10px;margin-bottom:16px;}
.cap-week-label{font-family:var(--fontc);font-size:13px;font-weight:700;color:var(--text);min-width:200px;text-align:center;}
...
// ══════════════════════════════════════════════════════════════════════════════
// PLANNING MODULE — Agenda · Calendario · Carga de trabajo
// ══════════════════════════════════════════════════════════════════════════════
function PlanningModule({ ots, users, onOpenOT }) {
  const [view,    setView]    = useState("agenda");   // agenda | calendar | capacity
  const [agendaDate, setAgendaDate] = useState(new Date());
  const [agendaRange, setAgendaRange] = useState("day");  // day | week
  const [calMonth, setCalMonth] = useState(new Date());
  const [calSelected, setCalSelected] = useState(null);
  const [capWeek, setCapWeek] = useState(getWeekStart(new Date()));
  const [filterTech, setFilterTech] = useState("all");

  const DAILY_HOURS = 8; // horas laborables por día y técnico
  ...
  const staff = users.filter(u => u.active && (u.role==="jefe"||u.role==="tecnico"));
  const activeOts = ots.filter(o => o.status!=="completed" && o.status!=="cancelled" && o.dueDate);

  // OTs filtradas por técnico
  const filteredOts = filterTech==="all" ? activeOts : activeOts.filter(o=>o.assignedId===filterTech);

  // ── OTs para un día concreto ──
  function otsForDay(date) {
    const iso = fmtIso(date);
    return filteredOts.filter(o => o.dueDate === iso);
  }

  // ── VISTA AGENDA ──
  const agendaDays = agendaRange==="day"
    ? [agendaDate]
    : Array.from({length:7}, (_,i) => addDays(getWeekStart(agendaDate), i));
  ...
  function calDays() {
    ...
    const days = [];
    for(let i=0; i<startDow; i++) days.push({date: addDays(first, -startDow+i), thisMonth:false});
    for(let i=1; i<=last.getDate(); i++) days.push({date: new Date(year,month,i), thisMonth:true});
    const rem = 7 - (days.length % 7);
    if(rem<7) for(let i=1; i<=rem; i++) days.push({date: addDays(last, i), thisMonth:false});
    return days;
  }

  // ── VISTA CARGA ──
  function capDays() {
    return Array.from({length:5}, (_,i) => addDays(capWeek, i)); // Lun-Vie
  }

  function hoursForTechDay(techId, date) {
    const iso = fmtIso(date);
    return ots
      .filter(o=>o.assignedId===techId && o.dueDate===iso && o.status!=="completed" && o.status!=="cancelled")
      .reduce((sum,o)=>(sum + (o.estimatedHours||0)), 0);
  }

  function hoursForTechWeek(techId) {
    return capDays().reduce((sum,d) => sum + hoursForTechDay(techId,d), 0);
  }

  const PRIORITY_COLOR = {critical:"var(--red)",high:"var(--orange)",medium:"var(--yellow)",low:"var(--green)"};
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,flexWrap:"wrap"}}>
        <div className="plan-tabs">
          {[
            ...
            {id:"capacity", label:"⚡ Carga de trabajo"},
          ].map(v=>(
            <div key={v.id} className={`plan-tab ${view===v.id?"on":""}`} onClick={()=>setView(v.id)}>
              {v.label}
            </div>
          ))}
        </div>
        <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center"}}>
          <span style={{fontSize:11,color:"var(--text2)"}}>Técnico:</span>
      ...
      {view==="agenda" && (
        <>
          <div className="agenda-nav">
            <button className="agenda-nav-btn" onClick={()=>setAgendaDate(d=>addDays(d, agendaRange==="day"?-1:-7))}>‹</button>
            <div className="agenda-date-label">
              {agendaRange==="day"
                ? fmt(agendaDate)
                : `${fmtShort(getWeekStart(agendaDate))} — ${fmtShort(addDays(getWeekStart(agendaDate),6))}`
              }
            </div>
            <button className="agenda-nav-btn" onClick={()=>setAgendaDate(d=>addDays(d, agendaRange==="day"?1:7))}>›</button>
            <button className="agenda-today-btn" onClick={()=>setAgendaDate(new Date())}>Hoy</button>
            <div style={{marginLeft:8,display:"flex",background:"var(--bg3)",borderRadius:6,overflow:"hidden",border:"1px solid var(--border)"}}>
              {[["day","Día"],["week","Semana"]].map(([k,l])=>(
                <div key={k} onClick={()=>setAgendaRange(k)}
                  style={{padding:"5px 12px",fontSize:11,fontWeight:600,cursor:"pointer",
                    background:agendaRange===k?"var(--bg5)":"transparent",
                    color:agendaRange===k?"var(--text)":"var(--text2)"}}>
                  {l}
                </div>
              ))}
            </div>
          ...
          <div className="agenda-cols" style={{gridTemplateColumns:`repeat(${agendaDays.length},1fr)`}}>
            {agendaDays.map(day => {
              const dayOts = otsForDay(day);
              const isToday = isSameDay(day, today);
              const totalH = dayOts.reduce((s,o)=>s+(o.estimatedHours||0),0);
              return (
                <div key={fmtIso(day)} className="agenda-col">
                  <div className={`agenda-col-head ${isToday?"today":""}`}>
                    <span>{day.toLocaleDateString("es-ES",{weekday:"short"}).toUpperCase()}</span>
                    <span className="agenda-col-head-date">{fmtShort(day)}</span>
                    {totalH>0 && <span style={{marginLeft:"auto",fontSize:9,fontFamily:"var(--mono)",background:"var(--bg4)",padding:"1px 6px",borderRadius:4,color:"var(--text2)"}}>{totalH}h</span>}
                  </div>
                  {dayOts.length===0 ? (
                    ...
                    dayOts
                      ...
                      .map(o => {
                        const tech = users.find(u=>u.id===o.assignedId);
                        return (
                          <div key={o.id} className={`agenda-slot ${o.priority}`}
                            onClick={()=>onOpenOT&&onOpenOT(o)}>
                            <div className="agenda-slot-id">{o.id}</div>
                            <div className="agenda-slot-title">{o.title}</div>
                            <div className="agenda-slot-meta">
                              {o.estimatedHours>0 && <span className="agenda-slot-hours">⏱ {o.estimatedHours}h</span>}
                              <span className="agenda-slot-asset">{o.asset}</span>
                              {tech && (
                                <span style={{display:"flex",alignItems:"center",gap:4,fontSize:10,marginLeft:"auto"}}>
                                  <span style={{width:16,height:16,borderRadius:"50%",background:tech.color||"#555",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,color:"#fff"}}>{tech.avatar}</span>
                                  <span style={{color:"var(--text2)"}}>{tech.name.split(" ")[0]}</span>
                                </span>
                              )}
                            </div>
                          </div>
      ...
      {view==="calendar" && (
        <>
          <div className="cal-nav">
            <button className="agenda-nav-btn" onClick={()=>setCalMonth(m=>{const d=new Date(m);d.setMonth(d.getMonth()-1);return d;})}>‹</button>
            <div className="cal-month-label">
              {calMonth.toLocaleDateString("es-ES",{month:"long",year:"numeric"}).replace(/^\w/,c=>c.toUpperCase())}
            </div>
            <button className="agenda-nav-btn" onClick={()=>setCalMonth(m=>{const d=new Date(m);d.setMonth(d.getMonth()+1);return d;})}>›</button>
            <button className="agenda-today-btn" onClick={()=>setCalMonth(new Date())}>Hoy</button>
          </div>
          ...
          <div className="cal-grid">
            ...
            {calDays().map(({date,thisMonth}) => {
              const dayOts = otsForDay(date);
              const isToday = isSameDay(date, today);
              const isSel   = calSelected && isSameDay(date, calSelected);
              const totalH  = dayOts.reduce((s,o)=>s+(o.estimatedHours||0),0);
              const loadPct = Math.min(100, (totalH/DAILY_HOURS)*100);
              const loadColor = loadPct>100?"var(--red)":loadPct>75?"var(--yellow)":"var(--green)";
              return (
                <div key={fmtIso(date)}
                  className={`cal-day ${isToday?"today":""} ${isSel?"selected":""} ${!thisMonth?"other-month":""}`}
                  onClick={()=>setCalSelected(isSel?null:date)}>
                  <div className="cal-day-num">{date.getDate()}</div>
                  <div className="cal-dot-row">
                    {dayOts.slice(0,5).map(o=>(
                      <div key={o.id} className="cal-dot" style={{background:PRIORITY_COLOR[o.priority]||"var(--text3)"}} title={o.title}/>
                    ))}
                    {dayOts.length>5 && <div style={{fontSize:8,color:"var(--text3)"}}>+{dayOts.length-5}</div>}
                  </div>
                  {dayOts.length>0 && <div className="cal-day-count">{dayOts.length} tarea{dayOts.length!==1?"s":""}</div>}
                  {totalH>0 && <div className="cal-load-bar" style={{background:loadColor,width:`${loadPct}%`}}/>}
                </div>
              );
            })}
          </div>
          ...
          {calSelected && otsForDay(calSelected).length>0 && (
            <div style={{marginTop:16,background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:8,padding:16}}>
              <div style={{fontSize:12,fontWeight:700,marginBottom:12,color:"var(--text2)",textTransform:"capitalize"}}>
                {fmt(calSelected)}
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {otsForDay(calSelected)
                  .sort((a,b)=>({critical:4,high:3,medium:2,low:1}[b.priority]||0)-({critical:4,high:3,medium:2,low:1}[a.priority]||0))
                  .map(o=>{
                    const tech = users.find(u=>u.id===o.assignedId);
                    return (
                      <div key={o.id} className={`agenda-slot ${o.priority}`} style={{margin:0}}
                        onClick={()=>onOpenOT&&onOpenOT(o)}>
                        <div className="agenda-slot-id">{o.id}</div>
                        <div className="agenda-slot-title">{o.title}</div>
                        <div className="agenda-slot-meta">
                          {o.estimatedHours>0&&<span className="agenda-slot-hours">⏱ {o.estimatedHours}h</span>}
                          <span className="agenda-slot-asset">{o.asset}</span>
                          {tech&&<span style={{marginLeft:"auto",fontSize:10,color:"var(--text2)"}}>{tech.name.split(" ")[0]}</span>}
                        </div>
                      </div>
                    );
                  })}
          ...
          {calSelected && otsForDay(calSelected).length===0 && (
            <div style={{marginTop:12,textAlign:"center",color:"var(--text3)",fontSize:12,padding:16}}>
              Sin tareas para {fmtShort(calSelected)}
            </div>
          )}
        </>
      )}
      ...
      {view==="capacity" && (
        <>
          <div className="cap-week-nav">
            <button className="agenda-nav-btn" onClick={()=>setCapWeek(w=>addDays(w,-7))}>‹ Semana anterior</button>
            <div className="cap-week-label">
              Semana {fmtShort(capWeek)} — {fmtShort(addDays(capWeek,4))}
            </div>
            <button className="agenda-nav-btn" onClick={()=>setCapWeek(w=>addDays(w,7))}>Semana siguiente ›</button>
            <button className="agenda-today-btn" onClick={()=>setCapWeek(getWeekStart(new Date()))}>Esta semana</button>
          </div>

          {/* Summary KPIs */}
          <div className="kpi-row" style={{marginBottom:16}}>
            {staff.map(u=>{
              const weekH = hoursForTechWeek(u.id);
              const maxH  = DAILY_HOURS * 5;
              const pct   = Math.round((weekH/maxH)*100);
              return (
                <div key={u.id} className="kpi">
                  <div className="kpi-label" style={{display:"flex",alignItems:"center",gap:6}}>
                    <span style={{width:14,height:14,borderRadius:"50%",background:u.color||"#555",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:7,fontWeight:700,color:"#fff",flexShrink:0}}>{u.avatar}</span>
                    {u.name.split(" ")[0]}
                  </div>
                  <div className="kpi-val" style={{color:pct>100?"var(--red)":pct>75?"var(--yellow)":"var(--green)",fontSize:22}}>{weekH}h</div>
                  <div style={{marginTop:6}}>
                    <div className="cap-bar-wrap">
                      <div className={`cap-bar ${pct>100?"over":pct>75?"warn":"ok"}`} style={{width:`${Math.min(100,pct)}%`}}/>
                    </div>
                    <div style={{fontSize:9,color:"var(--text3)",marginTop:3,fontFamily:"var(--mono)"}}>{pct}% de {maxH}h</div>
                  </div>
                  <div className="kpi-accent" style={{background:pct>100?"var(--red)":pct>75?"var(--yellow)":"var(--green)"}}/>
                </div>
              );
            })}
          </div>
          ...
          <div style={{background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:8,overflow:"hidden"}}>
            <table className="cap-table">
              <thead>
                <tr>
                  <th className="cap-th" style={{width:160}}>Técnico</th>
                  {capDays().map(d=>(
                    <th key={fmtIso(d)} className="cap-th center" style={{minWidth:100}}>
                      <div>{d.toLocaleDateString("es-ES",{weekday:"short"}).toUpperCase()}</div>
                      <div style={{fontFamily:"var(--mono)",fontSize:9,color:isSameDay(d,today)?"var(--amber)":"var(--text3)",fontWeight:isSameDay(d,today)?700:400}}>{fmtShort(d)}</div>
                    </th>
                  ))}
                  <th className="cap-th center">Total semana</th>
                </tr>
              </thead>
              <tbody>
                {staff.map(u=>(
                  <tr key={u.id}>
                    <td className="cap-td">
                      <div className="cap-tech">
                        <div style={{width:28,height:28,borderRadius:"50%",background:u.color||"#555",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"#fff",flexShrink:0}}>{u.avatar}</div>
                        <div>
                          <div style={{fontSize:12,fontWeight:600}}>{u.name}</div>
                          <div style={{fontSize:10,color:"var(--text3)"}}>{u.role==="jefe"?"Jefe":"Técnico"}</div>
                        </div>
                      </div>
                    </td>
                    {capDays().map(d=>{
                      const dayH = hoursForTechDay(u.id, d);
                      const dayOts = ots.filter(o=>o.assignedId===u.id&&o.dueDate===fmtIso(d)&&o.status!=="completed"&&o.status!=="cancelled");
                      const pct = Math.round((dayH/DAILY_HOURS)*100);
                      return (
                        <td key={fmtIso(d)} className="cap-td center">
                          {dayH===0 ? (
                            <span style={{color:"var(--text3)",fontSize:11}}>—</span>
                          ) : (
                            <div>
                              <div style={{marginBottom:4}}>
                                <div className="cap-bar-wrap" style={{width:60,margin:"0 auto"}}>
                                  <div className={`cap-bar ${pct>100?"over":pct>75?"warn":"ok"}`} style={{width:`${Math.min(100,pct)}%`}}/>
                                </div>
                              </div>
                              <div className={`cap-hours ${pct>100?"cap-over":""}`}>{dayH}h</div>
                              <div style={{fontSize:9,color:"var(--text3)"}}>{dayOts.length} OT{dayOts.length!==1?"s":""}</div>
                            </div>
                          )}
                        </td>
                      );
                    })}
                    <td className="cap-td center">
                      {(()=>{
                        const weekH = hoursForTechWeek(u.id);
                        const pct = Math.round((weekH/(DAILY_HOURS*5))*100);
                        return (
                          <div>
                            <div className={`cap-hours ${pct>100?"cap-over":""}`} style={{fontSize:13,fontWeight:700}}>{weekH}h</div>
                            <div style={{fontSize:9,color:pct>100?"var(--red)":pct>75?"var(--yellow)":"var(--text3)"}}>{pct}%</div>
                          </div>
                        );
                      })()}
                    </td>
          ...
          <div style={{fontSize:10,color:"var(--text3)",marginTop:8}}>
            * Basado en {DAILY_HOURS}h laborables/día · Solo OTs con fecha de vencimiento asignada
          </div>
        </>
      )}
    </div>
...
function BandejaRevision({ incidencias, users, currentUser, setIncidencias, setNotifs, onConvertToOT, addComment }) {
  ...
  return (
    <>
      ...
      <div className="g4" style={{marginBottom:16}}>
        {[
          {label:"Sin revisar",  key:"pending",   val:pendingList.length,    color:"var(--yellow)", stripe:"var(--yellow)"},
          {label:"En revisión",  key:"reviewing", val:reviewing_.length,     color:"var(--blue)",   stripe:"var(--blue)"},
          {label:"Aceptadas",    key:"accepted",  val:incidencias.filter(i=>i.status==="accepted"||i.status==="converted").length, color:"var(--green)", stripe:"var(--green)"},
          {label:"Rechazadas",   key:"rejected",  val:incidencias.filter(i=>i.status==="rejected").length, color:"var(--red)", stripe:"var(--red)"},
        ].map(k=>(
          <div key={k.label} className={`kpi ${kpiFilter===k.key?"active-filter":""}`}
            style={{cursor:"pointer"}} title={kpiFilter===k.key?"Quitar filtro":`Filtrar: ${k.label}`}
            onClick={()=>toggleKpiFilter(k.key)}>
            <div className="kpi-label">{k.label}</div>
            <div className="kpi-val" style={{color:k.color}}>{k.val}</div>
            <div className="kpi-accent" style={{background:k.stripe}}/>
          </div>
                ...
                <div className="page-header">
                  <div>
                    ...
                    <div className="page-sub">
                      {ots.filter(o=>o.status!=="completed"&&o.status!=="cancelled"&&o.dueDate).length} OTs planificadas ·{" "}
                      {users.filter(u=>u.active&&(u.role==="jefe"||u.role==="tecnico")).length} técnicos activos
                    </div>
                  </div>
                </div>
                <PlanningModule
